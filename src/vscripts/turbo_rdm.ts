...
export function InitGameMode(): void {
    ...
    mode.SetCustomXPRequiredToReachNextLevel({
        [1]: 0, [2]: 120, [3]: 160, [4]: 210, [5]: 260,
        [6]: 320, [7]: 380, [8]: 450, [9]: 520, [10]: 600,
        [11]: 690, [12]: 780, [13]: 880, [14]: 990, [15]: 1100,
        [16]: 1220, [17]: 1350, [18]: 1500, [19]: 1660, [20]: 1830,
        [21]: 2010, [22]: 2200, [23]: 2400, [24]: 2610, [25]: 2830,
        [26]: 3200, [27]: 3600, [28]: 4000, [29]: 4400, [30]: 4800,
    } as any);
    ...
    ListenToGameEvent("game_rules_state_change", () => OnGameStateChange(), undefined);

    // Client-to-server event: player picked a hero from the selection UI
    CustomGameEventManager.RegisterListener("turbo_rdm_hero_pick" as any, (_: number, event: any) => {
...
// ---------------------------------------------------------------------------
function SavePlayerInventory(hero: CDOTA_BaseNPC_Hero, playerID: PlayerID): void {
...
// ---------------------------------------------------------------------------
function RestorePlayerInventory(hero: CDOTA_BaseNPC_Hero, playerID: PlayerID): void {
...
function ApplyTurboBuildingModifiers(): void {
    ...
    const ancients = Entities.FindAllByClassname("npc_dota_fort") as CDOTA_BaseNPC[];
...
function OnGameStateChange(): void {
    ...
    if (state === DOTA_GameState.DOTA_GAMERULES_STATE_HERO_SELECTION) {
        for (let playerID = 0 as PlayerID; playerID < PlayerResource.GetPlayerCount(); playerID++) {
...
function OnNPCSpawned(event: NpcSpawnedEvent): void {
    ...
    if (pendingChoices[playerID] && !swappingPlayers[playerID]) {
        const chosen = pendingChoices[playerID].chosenHero
            || pendingChoices[playerID].heroes[0];
...
function OnEntityKilled(event: EntityKilledEvent): void {
    const killedUnit = EntIndexToHScript(event.entindex_killed) as CDOTA_BaseNPC_Hero;
    ...
    const player = PlayerResource.GetPlayer(playerID);
    if (player) {
        CustomGameEventManager.Send_ServerToPlayer(player, "turbo_rdm_hero_choices" as any, {
            hero1: choices[0],
            hero2: choices[1],
...
function OnHeroPicked(event: { PlayerID: PlayerID; hero_name: string }): void {
    ...
    const player = PlayerResource.GetPlayer(playerID);
    if (player) {
        CustomGameEventManager.Send_ServerToPlayer(player, "turbo_rdm_hero_chosen" as any, {});
    }
}
...
function ExecuteHeroSwap(playerID: PlayerID, heroName: string): void {
    ...
    PrecacheUnitByNameAsync(heroName, () => {
        ...
        if (newHero) {
            ...
            // Notify all players
            const msg = PlayerResource.GetPlayerName(playerID) + " has become " + heroName + "!";
            GameRules.SendCustomMessage(msg, 0, 0);

            CustomGameEventManager.Send_ServerToAllClients("turbo_rdm_hero_swap" as any, {
                player_id: playerID,
                hero_name: heroName,
            ...
            CustomGameEventManager.Send_ServerToPlayer(player, "turbo_rdm_hero_history" as any, {
            ...
            print(`[TurboRDM] Player ${playerID} swapped to ${heroName} at level ${targetLevel}`);
        } else {
